...
class CountyInfoUI(UI):
    def __init__(self, section, x, y, tiles):
        ...
        for type in Activities:
            ...
            tooltip = Tooltip(x=td[0], y=td[1], width=5, height=2, x_offset=-1, y_offset=2, text=activity_templates[type].description)
        ...
        for type in Policies:
            ...
            td = [startx, starty +(count * ygap)]
            tooltip = Tooltip(x=td[0], y=td[1], width=2, height=2, x_offset=-5, y_offset=2, text=policy_templates[type].description)
            self.add_element(tooltip)
